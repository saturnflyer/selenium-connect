...
describe SeleniumConnect::Config::Runner::Sauce do
  ...
  it 'should throw an exception if you ask for the api_key and it is nil' do
    expect do
      config.api_key
    end.to raise_error ArgumentError, 'The sauce api_key must be defined. Please do so.'
  end
  ...
  it 'it should merge in sym => value pairs' do
    ...
    sauce_config.api_key.should eq 'key'
